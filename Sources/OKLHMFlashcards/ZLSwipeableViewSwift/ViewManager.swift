...
class ViewManager: NSObject {
    ...
    var state: State {
        didSet {
            if case .snapping = oldValue, case let .moving(point) = state {
                unsnapView()
                attachView(toPoint: point)
            } else if case .snapping = oldValue, case let .swiping(origin, direction) = state {
                unsnapView()
                attachView(toPoint: origin)
                pushView(fromPoint: origin, inDirection: direction)
            } else if case .moving = oldValue, case let .moving(point) = state {
                moveView(toPoint: point)
            } else if case .moving = oldValue, case let .snapping(point) = state {
                detachView()
                snapView(point)
            } else if case .moving = oldValue, case let .swiping(origin, direction) = state {
                pushView(fromPoint: origin, inDirection: direction)
            } else if case .swiping = oldValue, case let .snapping(point) = state {
                unpushView()
                detachView()
                snapView(point)
            }
    ...
    @objc func handlePan(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            guard case .snapping = state else { return }
    ...
    fileprivate func attachView(toPoint point: CGPoint) {
        ...
        let p = view.center
        viewToAnchorViewAttachmentBehavior =
        UIAttachmentBehavior(
            item: view, offsetFromCenter: UIOffset(horizontal: -(p.x - point.x),
                                                   vertical: -(p.y - point.y)), attachedTo: anchorView, offsetFromCenter: UIOffset.zero)
    ...
    fileprivate func detachView() {
        guard let viewToAnchorViewAttachmentBehavior =
                viewToAnchorViewAttachmentBehavior,
              let anchorViewToPointAttachmentBehavior =
                anchorViewToPointAttachmentBehavior else { return }
        removeBehavior(viewToAnchorViewAttachmentBehavior)
        removeBehavior(anchorViewToPointAttachmentBehavior)
    }
    ...
    fileprivate func pushView(fromPoint point: CGPoint, inDirection direction: CGVector) {
        guard let _ = viewToAnchorViewAttachmentBehavior,
              let anchorViewToPointAttachmentBehavior = anchorViewToPointAttachmentBehavior
        else { return }
