...
// customization
public typealias AnimateViewHandler = (_ view: UIView, _ index: Int, _ views: [UIView], _ swipeableView: ZLSwipeableView) -> Void
public typealias InterpretDirectionHandler = (_ topView: UIView, _ direction: Direction,
                                              _ views: [UIView], _ swipeableView: ZLSwipeableView) -> (CGPoint, CGVector)
public typealias ShouldSwipeHandler = (_ view: UIView, _ movement: Movement, _ swipeableView: ZLSwipeableView) -> Bool

// delegates
public typealias DidStartHandler = (_ view: UIView, _ atLocation: CGPoint) -> Void
public typealias SwipingHandler = (_ view: UIView, _ atLocation: CGPoint, _ translation: CGPoint) -> Void
public typealias DidEndHandler = (_ view: UIView, _ atLocation: CGPoint) -> Void
public typealias DidSwipeHandler = (_ view: UIView, _ inDirection: Direction, _ directionVector: CGVector) -> Void
public typealias DidCancelHandler = (_ view: UIView) -> Void
public typealias DidTap = (_ view: UIView, _ atLocation: CGPoint) -> Void
public typealias DidDisappear = (_ view: UIView) -> Void
...
open class ZLSwipeableView: UIView {
    ...
    open var numberOfHistoryItem: UInt {
        return UInt(history.count)
    }
    ...
    fileprivate var containerView = UIView() // It is responsible for holding and managing the card views added to the ZLSwipeableView
    ...
    open func loadViews() {
        // when activeViews are < numberOfActiveView i.e 4 in this case
    ...
    open func discardViews() {
        ...
        remove(topView)
        history.append(topView) // Add the discarded card to the history
        loadViews()
    }
    ...
    // as soon as views become < numberOfActiveViews(4), this function will be called
    fileprivate func insert(_ view: UIView, atIndex index: Int) {
        ...
        let viewManager = ViewManager(view: view, containerView: containerView,
                                      index: index, miscContainerView: miscContainerView, animator: animator, swipeableView: self)
    ...
    open func updateViews() {
        ...
        guard let gestureRecognizers = activeViews.first?.gestureRecognizers, gestureRecognizers.filter({ gestureRecognizer in
            gestureRecognizer.state != .possible }).isEmpty
        else { return }

        for item in 0 ..< activeViews.count {
            let view = activeViews[item]
            view.isUserInteractionEnabled = onlySwipeTopCard ? item == 0 : true
            let shouldBeHidden = item >= Int(numberOfActiveView)
            view.isHidden = shouldBeHidden
            guard !shouldBeHidden else { continue }
            animateView(view, item, activeViews, self)
        }
    }
...
extension ZLSwipeableView {
    ...
    static func defaultAnimateViewHandler() -> AnimateViewHandler {
        ...
        func rotateView(_ view: UIView, forDegree degree: CGFloat, duration: TimeInterval, offsetFromCenter offset: CGPoint, swipeableView: ZLSwipeableView, completion: ((Bool) -> Void)? = nil) {
            UIView.animate(withDuration: duration, delay: 0, options: .allowUserInteraction, animations: {
                // update center of view to swipeable view
                ...
                // for getting view to original position
    ...
    static func defaultInterpretDirectionHandler() -> InterpretDirectionHandler {
        return { (topView: UIView, direction: Direction, views: [UIView], swipeableView: ZLSwipeableView) in
            ...
            switch direction {
            case Direction.Left:
                directionVector = CGVector(dx: -programmaticSwipeVelocity, dy: 0) // -,0 // x,y format
            case Direction.Right:
                directionVector = CGVector(dx: programmaticSwipeVelocity, dy: 0) // +,0
            case Direction.Up:
                directionVector = CGVector(dx: 0, dy: -programmaticSwipeVelocity) // 0,-
            case Direction.Down:
                directionVector = CGVector(dx: 0, dy: programmaticSwipeVelocity) // 0,+
            default:
...
extension CGPoint {
    ...
    static func areInSameTheDirection(_ p1: CGPoint, p2: CGPoint) -> Bool {
        ...
        func signNum(_ number: CGFloat) -> Int {
